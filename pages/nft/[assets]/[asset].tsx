...
import { GET_USER, UserDataType } from "..";
...
export default function Asset() {
  ...
  const {
    ...
    error: apolloError,
    data: apolloData,
  } = apolloQuery<UserDataType>(GET_USER, {
    variables: {
      input: {
  ...
  const { error, data, isLoading } = reactQuery<AssetType>("asset", () =>
    fetch(
      `https://api.opensea.io/api/v1/asset/${tokenAddress}/${tokenId}`
    ).then((res) => res.json())
  );

  if (error ?? apolloError) return <CustomError error={error as Error} />;
  if (isLoading ?? loading ?? !data ?? !apolloData) return <Loader />;
